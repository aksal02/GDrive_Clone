<header class="flex justify-between items-center bg-bri-blue text-white py-6 px-6 rounded-2xl shadow-lg relative"
        style="
    ...
    <div class="absolute inset-0 bg-bri-blue rounded-2xl shadow-lg overflow-hidden">
        <div class="absolute inset-0"
             style="
                 ...
                 background-position: center;
             "></div>
        <div class="absolute inset-0 bg-bri-blue opacity-50"></div>
    </div>
    <div class="relative z-10 flex justify-between items-center w-full">
        <!-- Form Pencarian -->
        <form method="GET" action="{{ url()->current() }}" class="w-64">
            <div class="relative">
                <span class="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-gray-700">search</span>
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search in DataBOX..." 
                       class="w-full bg-white/70 text-gray-800 placeholder-gray-500 border-none rounded-full py-2 pl-12 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-white/50 transition">
            </div>
        </form>
